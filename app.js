...
const over1500 = (inventor) => {
  if(inventor.year >= 1500 && inventor.year <= 1599) {
    return true;
  }
}
veryOldInventors = inventors.filter(over1500)
...
const setName = (person) => {
  let modName =  person.split(', ') // split needs to be stored in a variable
  return `${modName[1]} ${modName[0]}`
}
// firstLast = people.split(' , ')
firstLast = people.map(setName)
// Check your work:
...
const is19OrOlder = (dev) => {
  return dev.year <= 2005;
}
isEveryone19OrOlder = devs.every(is19OrOlder)
